...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series INNER JOIN characters ON characters.series_id = series.id ORDER BY COUNT(characters.species) DESC LIMIT 1 GROUP BY (characters.species) HAVING characters.species = 'human' "
end
